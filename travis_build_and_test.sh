...
if [ "${TEST_TYPE}" == "acceptance-test" ]; then
  if [ "${TRAVIS_PULL_REQUEST}" == "false" -a "${TRAVIS_BRANCH}" == "travisSauceLabs" ]; then
...
elif [ "${TEST_TYPE}" == "examples" -a "${TRAVIS_PULL_REQUEST}" == "false" -a "${TRAVIS_BRANCH}" == "travisSauceLabs" ]; then
